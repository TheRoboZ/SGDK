...
NO_INLINE void SPR_initEx(u16 vramSize)
{
    ...
    KLog("Sprite engine (fast) initialized !");
    ...
    for(u16 indAnim = 0; indAnim < numAnimation; indAnim++)
    {
        ...
        *animFrameIndexes++ = frameIndexes;
        ...
        for(u16 indFrame = 0; indFrame < numFrame; indFrame++)
        {
            ...
            if (ind == tileInd)
            {
                // count tile and pass to next tileset
                tileInd += tileset->numTile;
            }
            // store frame tile index
            *frameIndexes++ = ind;
...
void SPR_setHFlip(Sprite* sprite, bool value)
{
    ...
    if (value) attr |= TILE_ATTR_HFLIP_MASK;
    else attr &= ~TILE_ATTR_HFLIP_MASK;

    if (attr != sprite->attribut)
    {
        // update attribut (flip stream selection is done during SPR_update())
...
void SPR_setVFlip(Sprite* sprite, bool value)
{
    ...
    if (value) attr |= TILE_ATTR_VFLIP_MASK;
    else attr &= ~TILE_ATTR_VFLIP_MASK;
...
void SPR_setPriority(Sprite* sprite, bool value)
{
    ...
    if (value) sprite->attribut = attr | TILE_ATTR_PRIORITY_MASK;
    else sprite->attribut = attr & ~TILE_ATTR_PRIORITY_MASK;
...
void SPR_setVisibility(Sprite* sprite, SpriteVisibility value)
{
    ...
    switch(value)
    {
        case VISIBLE:
            ...
            sprite->visibility = VISIBILITY_ON;
            break;

        case HIDDEN:
            status &= ~(SPR_FLAG_AUTO_VISIBILITY | NEED_VISIBILITY_UPDATE);
            sprite->visibility = VISIBILITY_OFF;
            break;
    ...
    while(sprite)
    {
        ...
        if (sprite->visibility && (status & ALLOCATED) && (vdpSpriteInd <= SAT_MAX_SIZE))
        {
            ...
            else
            {
                ...
                if (attr & TILE_ATTR_HFLIP_MASK) tmplBase += streamSize;
                if (attr & TILE_ATTR_VFLIP_MASK) tmplBase += streamSize << 1;
            }

            const u16* tmpl = (const u16*) tmplBase;

            // emit SAT entries: template data is stored in SAT field order,
...
static u16 updateVisibility(Sprite* sprite, u16 status)
{
    ...
    // compute global visibility for the whole (meta) sprite ('unsigned' allow merged <0 test)
